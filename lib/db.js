...
function resolveDbPath() {
  const candidates = [
    ...
    path.join(dataDir, 'writeflow.db'),
    path.join('/tmp', 'writeflow.db'),
  ].filter(Boolean);
  ...
  for (const candidate of candidates) {
    ...
    try {
      ...
      console.warn(`DB path not usable (${candidate}): ${err.message}`);
    }
  }

  throw new Error('No writable database path found');
}
...
db.pragma(`journal_mode = ${process.env.NODE_ENV === 'production' ? 'DELETE' : 'WAL'}`);
